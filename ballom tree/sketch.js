...
function drawTree(x1, y1, angle, depth) {
  if (depth > 0) {
    ...
    let spread = map(mouseX, 0, width, 5, 15); // Map mouseX to angle spread between 5 and 45 degrees
    ...
    if (depth < 5) {
      noStroke();
      let leafSize = map(depth, 1, 5, 10, 30);  // Leaf size smaller at deeper levels
